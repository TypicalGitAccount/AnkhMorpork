...
namespace Ankh_Morpork.Events
{
    ...
    {
        protected BeggarRewardCents randomPractice()
        {
            Array values = Enum.GetValues(typeof(BeggarRewardCents));
            return (BeggarRewardCents)values.GetValue(rand.Next(values.Length));
        }
        ...
        public override bool Run(GameTools.User user, InputProcessor inputProcessor, OutputProcessor outputProcessor)
        {
            ...
            var interactionCostDollars = CurrencyConverter.CentsToDollars(beggar.State.InteractionCostCents);
            ...
            outputProcessor.Output($"Pocket: { CurrencyConverter.CentsToDollars(user.BalanceCents)}$\n\n"  +
                ...
                $"It looks like if you don\'t give him alms ({interactionCostDollars} $), he will chase you to death..\n" +
